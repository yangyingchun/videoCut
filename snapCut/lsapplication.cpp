...
void LSApplication::setImageAndWinName( const Mat& _image, const string& _winName  )
{
    ...
    images = _image;
    winName = &_winName;
    mask.create( images.size(), CV_8UC1);
    reset();
}
...
void LSApplication::showImage() const
{
    if (images.empty() || winName->empty())
        return;

    Mat res;

    Mat binMask = mask & 1;
    images.copyTo(res, binMask);
...
void LSApplication::mouseClick( int event, int x, int y, int flags, void* )
{
    ...
    case CV_EVENT_LBUTTONUP:
        if( lblsState == IN_PROCESS )
        {
			...
			if((flags & BGD_KEY) != 0)
			{
			    lazySnapping(images, mask, bgdModel, fgdModel);
                showImage();
			}
...
bool LSApplication::checkMask(Mat& mask)
{
...
void LSApplication::initlearningGMMs(const Mat& img, const Mat& mask, GMM& bgdGMM, GMM& fgdGMM) {
...
double LSApplication::calcBeta(const Mat& img) {
    double beta = 0;
...
void LSApplication::calcNWeights(const Mat& img, Mat& leftW, Mat& upleftW, Mat& upW,
...
void LSApplication::constructGCGraph(const Mat& img, const Mat& mask, const GMM& bgdGMM,
...
void LSApplication::estimateSegmentation(GraphType* graph, Mat& mask) {
    graph->maxflow();
    Point p;
    for (p.y = 0; p.y < mask.rows; p.y++) {
        for (p.x = 0; p.x < mask.cols; p.x++) {
...
void LSApplication::lazySnapping(const Mat& img, Mat& mask, Mat& bgdModel,
        Mat& fgdModel){
