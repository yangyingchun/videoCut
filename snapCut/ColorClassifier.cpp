...
//建立窄带的混合高斯模型
int ColorClassifier::BuildBand()
{

	Size imageS = mImageF.size ();
...
int ColorClassifier::Build()
{
	...
	if(mF_C > 0.85 && mF_C <= 1)
	{
		float a = ((float)VideoSnapCut::mMaskSize - 2.0)/((1.0 - 0.85)*(1.0 - 0.85));
		sigamS = 2.0 + a*(mF_C - 0.85)*(mF_C - 0.85);
	}
	else if(mF_C >= 0 && mF_C <= 0.85)
	{
		sigamS = 2.0;
	}
