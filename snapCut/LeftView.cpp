...
void CLeftView::SetImageMat(const Mat& mFrameMat)//将mat矩阵变为IPLImage
{
	ClearForegroundBorderPolygons();
...
void CLeftView::LoadImageFile()//最先执行
{
	UpdateData();	
	...
	else // folder
	{
		if(mFilesPath.size()>0)
		{
			...
			mVideoSnapCut->SetFilesPath(&mFilesPath);//传路径

			mVideoSnapCut->SetCurFrame(mCurFrame);//穿当前帧编号
		}//if(mFilesPath.size()>0)
...
void CLeftView::AddForegroundBorder(PolygonF *borderPolygon)
{
	...
	mForegroundBorderPolygons.push_back(borderPolygon);	//将边界多边形加入到容器中
}
...
void CLeftView::OnBnClickedButtonSampleContour()
{
	...
	mVideoSnapCut->CreateMasks();//之前去掉了
...
{
	mVideoSnapCut->SetForegroundBorderPolygons(&mForegroundBorderPolygons);//将leftview中的轮廓线传到snapcut中
	...
	imwrite("e:\\CombinedMask.jpg", mVideoSnapCut->mCombinedMask);//之前去掉了
